...
function sourceResponse(url: string): WebAccessResponse {
  const bytes = new TextEncoder().encode("public collectible research evidence");
  let consumed = false;
  return {
    status: 200,
    statusText: "OK",
  ...
  try {
    ...
    const memoryResponse = await fetch(`${baseUrl}/v1/brain/memory/select`, {
      ...
      body: JSON.stringify({
        ...
        query: "signed jersey authentication evidence",
        memories: [
          {
            id: "memory-1",
            type: "semantic",
            summary: "signed jersey authentication evidence",
    ...
    assert(research.sources[0]?.content === "public collectible research evidence", "Research HTTP route lost source content.");
