...
async function main(): Promise<void> {
  ...
  const mission: Mission = {
    ...
    name:
      "KCM real execution",
    description:
      "Prove the coding machine can drive the real Linux execution pipeline.",
    status:
      "active",
    objectives: [
      "Execute one real governed engineering operation.",
    ],
    sourceReferences: [
      "test://kcm-real-execution",
    ],
    createdAt:
      now,
    updatedAt:
  ...
  const plan: MissionPlan = {
    ...
    decisionIds: [],
    acceptanceCriteria: [
      "The real command executes successfully.",
    ],
    locked:
      false,
    approvedByHuman:
  ...
  assert(
    verified.verified,
    "javascript toolchain should verify",
  );
  ...
  assert(
    !engineeringPlan.blocked,
    "engineering plan must be unblocked",
  );
  ...
  assert(
    ...
    "engineering execution has no current step",
  );
  ...
  console.log(
    "PIPELINE STATUS:",
    result.pipeline.execution.status,
  );

  console.log(
    "EXIT CODE:",
    result.pipeline.step.exitCode,
  );

  console.log(
    "STDOUT:",
    JSON.stringify(
      result.pipeline.step.stdout,
    ),
  );

  assert(
    result.pipeline.realExecution,
    "machine did not reach real execution",
  );

  assert(
    result.pipeline.execution.status ===
      "completed",
    "real execution did not complete",
  );

  assert(
    result.pipeline.step.exitCode ===
      0,
    "real command did not exit successfully",
  );

  assert(
    result.pipeline.step.stdout.includes(
      "KCM_REAL_EXECUTION_GREEN",
    ),
  ...
  assert(
    result.execution.completedStepIds.length ===
      1,
    "machine did not advance execution state",
  );
  ...
  console.log(
    "K.I.N.G.S. MACHINE → REAL LINUX EXECUTION: SUCCESS",
  );

  console.log(
    "K.I.N.G.S. MACHINE → EXECUTION STATE ADVANCE: SUCCESS",
  );

  console.log(
    "K.I.N.G.S. MACHINE → MISSION STATE UPDATE: SUCCESS",
  );

  console.log(
    "TREE-KCM-EXECUTION: SUCCESS",
  );
}
